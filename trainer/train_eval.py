...
def train_ae(model, train_dl, optimizer, criterion,config,device):
    model.train()
    epoch_loss = 0
    epoch_score = 0
    Recons = RMSELoss()
    for inputs, labels in train_dl:
        ...
        pred, feat, reconst = model(src)

        #loss and score
        rul_loss = criterion(pred.squeeze(), labels)
        recon_loss = Recons(src, reconst)
        loss = rul_loss + recon_loss
